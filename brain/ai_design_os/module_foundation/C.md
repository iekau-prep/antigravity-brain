...
以下の運用でFoundation Packを進めます。

・Foundation Packで指定された.mdは、1ファイルずつ共有します。
・各.md読了後、簡潔な理解メモと次に読み込む.md名を返却してください。
・理解メモは記憶定着と現在理解の確認のみを目的とします。
・こちらからフィードバック・補正・評価は行いません。
・Foundation Pack全体完了後に、Stage（１〜６）単位の理解確認を実施します。
・推測は禁止します。不明点は保持せず、その時点で停止してください。
