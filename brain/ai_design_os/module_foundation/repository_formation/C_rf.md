# C_rf｜Repository Formation GPT Foundation Knowledge Pack

## Purpose

本書は、Repository Formation GPTがPJを推測せず理解し、
PJ内SoTを根拠として判断できる状態を形成するための知識基盤と読み込み順を提供する。

本書はAIの動作仕様ではない。

Repository Formation GPT固有のRole、Authority、Current State、
Incoming Request、Repository Formation対象、Handoffは定義しない。
...
- 過去の重要Formation、Legacy表現、Remaining Gap、またはCurrent BoundaryへのTraceabilityが判断上必要な案件では、`brain/design_formation_knowledge/index.md`
