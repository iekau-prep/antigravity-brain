...
④ LINE接続を前提にするが、今は仮user_idで成立させる
...
👉 後でLINE接続により、既存の本人の利用・判断形成を継続利用へ接続できる状態にする

この接続は、本人IdentityをLINE user_idへ置換すること、または判断・判断材料を再生成することを意味しない。仮`user_id`とLINE user_idの関係・接続方法はここで確定しない。
